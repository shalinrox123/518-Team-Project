...
public class GreetingController {
    ...
    public ModelAndView get_weather_city_name(@PathVariable String LOCATION) throws ParseException {
        ...
        try {
            ...
            double result_lon = firstJSONObject_coord.getDouble("lon");        // ************** important
            double result_lat = firstJSONObject_coord.getDouble("lat");        // ************** we'll need these

            // Now call One Call API

            String oneCallURL = "http://api.openweathermap.org/data/2.5/onecall?lat=" + result_lat + "&lon=" + result_lon + "&appid=" + API_KEY + "&units=imperial";


            try {
                URL url2 = new URL(oneCallURL);
                ...
                // ************** //
                JSONObject JSONObject_openWeatherAPIResponse = new JSONObject(json2);
            ...
            */
                ...
                for (int i = 0; i < numOfHours; i++)
                {
                    ...
                    hourly_dt_adjusted_daydate[i] = getDayDate(hourly_dt_adjusted[i]);
                    hourly_dt_adjusted_hour[i] = getHour(hourly_dt_adjusted[i]);
                ...
                for (int i = 0; i < numOfDays; i++)
                {
                    ...
                    daily_dt_adjusted_hour[i] = getHour(daily_dt_adjusted[i]);
                    daily_dt_adjusted_daydate[i] = getDayDate(daily_dt_adjusted[i]);

                    daily_sunrise[i] = JSONArray_daily.getJSONObject(i).getInt("sunrise");
                    daily_sunrise_adjusted[i] = daily_sunrise[i] + timezone_offset;
                    daily_sunrise_adjusted_hour[i] = getHour(daily_sunrise_adjusted[i]);
                    daily_sunrise_adjusted_daydate[i] = getDayDate(daily_sunrise_adjusted[i]);

                    daily_sunset[i] = JSONArray_daily.getJSONObject(i).getInt("sunset");
                    daily_sunset_adjusted[i] = daily_sunset[i] + timezone_offset;
                    daily_sunset_adjusted_hour[i] = getHour(daily_sunset_adjusted[i]);
                    daily_sunset_adjusted_daydate[i] = getDayDate(daily_sunset_adjusted[i]);
    ...
    public ModelAndView get_futurecast(@PathVariable String LATLON) throws ParseException {
        ...
        try {
            ...
            */
            ...
            //returnPage.setViewName("mainpage"); *********
            returnPage.setViewName("futurecast");
    ...
    public ModelAndView get_futurecast_1(@PathVariable String LATLON) throws ParseException {
        ...
        try {
            ...
            */
            ...
            //returnPage.setViewName("mainpage"); *********
            returnPage.setViewName("futurecast-logged-in");
